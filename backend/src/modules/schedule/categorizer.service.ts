...
export function categoryToSummary(category: ScheduleCategory): ScheduleCategorySummary {
  return {
    id: category.id,
    name: category.name,
...
export async function categorizeSchedule(items: unknown[]): Promise<CategorizedScheduleItem[]> {
...
export async function filterVisibleSchedule(items: unknown[]): Promise<unknown[]> {
  const categories = await getAllCategories();
