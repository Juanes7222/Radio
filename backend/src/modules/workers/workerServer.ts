...
import type { WorkerMessage, RegisterMessage } from "./protocol.types";
import {
  registerWorker,
...
import { handleJobDone, handleJobError, handleJobStatus } from "./jobDispatcher";
...
export function startWorkerServer(): WebSocketServer {
  ...
  wss.on("connection", (socket: WebSocket, req: IncomingMessage) => {
    ...
    socket.on("message", async (raw) => {
      ...
      switch (message.type) {
        ...
        case "pong":
          // Only updates lastSeenAt, does not mutate the status
          updateWorkerHeartbeat(message.workerId);
          break;

        case "job_ack":
          logger.info("WorkerServer", "Job acknowledged", {
            jobId: message.jobId,
  ...
  setInterval(() => {
    pruneDeadWorkers(config.worker.heartbeatTimeoutMs);
  }, PING_INTERVAL_MS);
