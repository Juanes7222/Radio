...
const buildShareLinks = (url: string, text: string) => ({
...
export function ShareModal({ open, onOpenChange, stationName }: ShareModalProps) {
  const [copied, setCopied] = useState(false);

  const url = window.location.href;
  const text = `Escucha ${stationName} - Tu emisora online 24/7`;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Compartir {stationName}</DialogTitle>
          <DialogDescription>
            Comparte la emisora en tus redes o copia el enlace directo.
          </DialogDescription>
        </DialogHeader>

        <div className="grid grid-cols-4 gap-2 py-4">
          {networks.map(({ label, icon: Icon, action, className }) => (
            <button
              key={label}
              onClick={action}
              className={`flex flex-col items-center gap-2 p-3 rounded-xl border transition-colors duration-200 text-sm font-medium ${className}`}
            >
              <Icon />
