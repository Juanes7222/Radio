...
const HOUR_WORDS: Record<number, string> = {
  ...
  6: 'seis', 7: 'siete', 8: 'ocho', 9: 'nueve', 10: 'diez',
  11: 'once', 12: 'doce',
  13: 'una', 14: 'dos', 15: 'tres', 16: 'cuatro', 17: 'cinco',
  18: 'seis', 19: 'siete', 20: 'ocho', 21: 'nueve', 22: 'diez',
  23: 'once',
};

const PERIOD_WORDS: Record<number, string> = {
  0: 'de la noche',
  1: 'de la madrugada', 2: 'de la madrugada', 3: 'de la madrugada',
  4: 'de la madrugada', 5: 'de la madrugada',
  6: 'de la mañana', 7: 'de la mañana', 8: 'de la mañana',
  9: 'de la mañana', 10: 'de la mañana', 11: 'de la mañana',
  12: 'del mediodía',
  13: 'de la tarde', 14: 'de la tarde', 15: 'de la tarde',
  16: 'de la tarde', 17: 'de la tarde', 18: 'de la tarde',
...
function getTimeText(hour24: number, minutes: number): string {
  if (minutes <= 30) {
    const hourWord = HOUR_WORDS[hour24] ?? `${hour24}`;
    const periodStr = PERIOD_WORDS[hour24] ?? '';
    const isOne = hour24 % 12 === 1;
  ...
  const hourWord = HOUR_WORDS[nextHour] ?? `${nextHour}`;
  const periodStr = PERIOD_WORDS[nextHour] ?? '';
  const isOne = nextHour % 12 === 1;
  const verb = isOne ? 'Es' : 'Son';
  const article = isOne ? 'la' : 'las';
  const minutesPhrase = numberToSpanishMinutes(minutes);
  return `${verb} ${article} ${hourWord} ${minutesPhrase} ${periodStr}`;
