...
export function mergeWithLocalEnv(
  ...
  for (const [key, value] of Object.entries(infisicalSecrets)) {
    ...
    if (isLocalEmpty) {
      ...
      skipped++;
      console.log(
        `[Infisical] Skipping ${key}: already defined locally (${localValue})`
      );
    }
  }
...
export function getInfisicalConfigFromEnv(): InfisicalOptions | null {
